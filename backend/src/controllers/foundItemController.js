...
async function createFoundItem(req, res) {
  try {
    const { title, category, description, location, found_date, contact, image_urls } = req.body;
    if (!title || !category || !description || !location || !found_date || !contact) {
      return res.status(400).json({ message: "请完整填写招领信息" });
    }

    const imageUrls = normalizeImageUrls(image_urls);
    const [result] = await pool.query(
      `INSERT INTO found_items (title, category, description, location, found_date, contact, image_urls, user_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [title, category, description, location, found_date, contact, JSON.stringify(imageUrls), req.user.id]
    );

    return res.status(201).json({ id: result.insertId, message: "招领信息发布成功" });
  } catch (error) {
    return res.status(500).json({ message: "发布失败", detail: error.message });
  }
}

async function listFoundItems(req, res) {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT fi.*, u.username
       ...
       WHERE (? = '' OR fi.title LIKE CONCAT('%', ?, '%') OR fi.description LIKE CONCAT('%', ?, '%'))
         AND (? = '' OR fi.category = ?)
         AND (? = '' OR fi.status = ?)
...
async function updateFoundItemStatus(req, res) {
  try {
    ...
    if (result.affectedRows === 0) {
      return res.status(404).json({ message: "招领信息不存在" });
    }
    return res.json({ message: "招领状态更新成功" });
  } catch (error) {
